...
# Latsly, it save all the information in a CSV file named 'Addresses.csv'
...
def save_address(address, street, house):
    """
    ...
    dicctionaryToDB["housenumber"].append(house)
...
def address_house_separator(address):
    ...
    if ',' in address:
        ...
        house = min(lis, key=len).strip()
        street = max(lis, key=len).strip()
        ...
        for x in range(1, len(address)):
            if address[x-1].isnumeric() == True and address[x].isnumeric() == False:
                ...
                house = min(lis, key=len)
                break

    else:
        if address[0].isdigit():
            house = address.split()[0].strip()
            ...
            for x in range(len(address)):
                if address[x].isnumeric():
                    ...
                    house = address[x:].strip()
                    break
    
    save_address(address, street, house)
    return print(json.dumps({"street":street, "housenumber":house}))
